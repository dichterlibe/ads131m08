 ...
 * MACRO DEFINE
 */
#define	LIST_COUNT(listp)	((listp)->count)
#define	LIST_LOCK(listp)	spin_lock(&((listp)->lock))
#define	LIST_UNLOCK(listp)	spin_unlock(&((listp)->lock))
#define	LIST_ELM_LOCK(elmp)	spin_lock(&((list_elm_t *)(elmp))->lock)
#define	LIST_ELM_UNLOCK(elmp)	spin_unlock(&((list_elm_t *)(elmp))->lock)
...
extern void list_cleanup(list_t *list,void (*free_elm)(const void *elm));
extern void list_purge(list_t *list,void (*free_elm)(const void *elm));
extern void list_iter_do(list_t *list,void (*action)(const void *elm));
...
extern list_t *list_new(struct device *dev);
